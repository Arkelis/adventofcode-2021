...
let part2 =
  let rec get_last predicate binaries index =
    let count_ones binaries index =
      let to_int bin =
        String.get bin index |> int_of_bit in
      List.map ~f:to_int binaries |> List.fold ~f:(+) ~init:0 in

    let ceil_half n = 
      int_of_float @@ Float.round_up @@ (float_of_int n) /. 2. in

    match binaries with
    | [result] -> result
    | more -> begin 
        let threshold = ceil_half (List.length binaries)
        and count = count_ones binaries index in
        get_last 
          predicate 
  ...
  let filter_most_common threshold count index binary =
    ...
    Char.equal char @@ if count >= threshold then '1' else '0'
  ...
  and last_with_least_commons =  get_last filter_least_common bins 0 in
